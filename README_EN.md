...
After ECO and the final save, static analysis found 243 components: 147 on the top side and 96 on the bottom side. The final data contains 316 nets, 1,155 pads, 4,928 tracks, 13 vias, and 557 unrouted connections. The reduction of 256 pads exactly matches the removed second FBGA256 footprint.
...
| DRC | Actual result for the reconstructed PCB | 2,640 violations; see the category breakdown below |
...
The complete DRC was run in Altium Designer 25.8.1 with the stop limit raised to 10,000 so that the default 500-violation limit did not truncate the result. It reported 2,640 violations:
...
The duplicate FPGA repair is complete, but the reconstructed PCB still requires routing, silkscreen, and manufacturing-clearance cleanup. It is not a DRC-clean production release.
