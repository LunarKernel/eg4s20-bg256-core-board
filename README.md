...
完成 ECO 并保存后的最终解析结果为 243 个 Component，其中 Top 147、Bottom 96；当前数据包含 316 个网络、1155 个焊盘、4978 条走线、13 个过孔和 557 个连接记录，最新 DRC 中有 547 条未布线。焊盘减少 256 个，恰好对应被删除的第二个 FBGA256 封装。
...
| DRC | 记录新工程真实违规数 | 2618 条活动违规、12 条已记录豁免；详见下方分类，未通过量产验收 |
| 工程可移植性 | 从最终路径打开，无缺失文档或库 | 通过；全部工程文档和 IntLib 可加载 |

完整 DRC 在 Altium Designer 25.8.1 中运行，检查上限提高到 10000，未因默认 500 条上限提前终止。调试前结果为 2640 条。对照课程文档中的正反面实物图后，确认 P2/P5 是正面已装连接器，P3/P6 是同位保留的背面未装替代焊盘；删除或移动任一组都会降低实物复刻一致性。因此没有修改布局或放宽全局规则，而是仅对这四个标号之间已证实的重叠记录 12 条带原因的 DRC 豁免。

在不移动元器件的前提下，继续补齐了 10 个经网络名和端点确认的断点：DONE 的局部段及 U7-P13 段、PROGRAM B 的局部三段及 U7-T2 段、INIT B 到 U7-R3，以及 MSEL0、MSEL1 和 FGPA GCLK1 的 FPGA 配置连接。每次修改后均完整复跑 DRC，未新增 Clearance、Short-Circuit 或 Hole To Hole 违规。

当前结果为 2618 条活动违规和 12 条已记录豁免：

- Clearance：0 条活动违规，4 条已豁免；
- Short-Circuit：0 条活动违规，6 条已豁免；
- Un-Routed Net：547；
- Hole To Hole Clearance：0 条活动违规，2 条已豁免；
- Minimum Solder Mask Sliver：227；
- Silk To Solder Mask：1442；
- Silk To Silk：402。

豁免原因已写入 PCB 文件，完整 DRC 复跑后仍能重新识别，且报告继续列出 Waived Violations。全部 12 条已知连接器重叠均采用精确豁免，没有降低任何全局规则。完整复刻板仍需要后续布线、丝印与工艺间距整理，不能作为可直接投板的 DRC-clean 版本。
