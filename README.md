...
| DRC | 记录新工程真实违规数 | 2630 条活动违规、10 条已记录豁免；详见下方分类，未通过量产验收 |
| 工程可移植性 | 从最终路径打开，无缺失文档或库 | 通过；全部工程文档和 IntLib 可加载 |

完整 DRC 在 Altium Designer 25.8.1 中运行，检查上限提高到 10000，未因默认 500 条上限提前终止。调试前结果为 2640 条。对照课程文档中的正反面实物图后，确认 P2/P5 是正面已装连接器，P3/P6 是同位保留的背面未装替代焊盘；删除或移动任一组都会降低实物复刻一致性。因此没有修改布局或放宽全局规则，而是仅对这四个标号之间已证实的重叠记录 10 条带原因的 DRC 豁免。

调试后的结果为 2630 条活动违规和 10 条已记录豁免：
...
豁免原因已写入 PCB 文件，完整 DRC 复跑后仍能重新识别，且报告继续列出 Waived Violations。两条 P2/P3 定位孔的 Hole To Hole Clearance 仍保持活动状态，没有通过降低全局孔间距要求掩盖。完整复刻板仍需要后续布线、丝印与工艺间距整理，不能作为可直接投板的 DRC-clean 版本。
