...
完成 ECO 并保存后的最终解析结果为 243 个 Component，其中 Top 147、Bottom 96；当前数据包含 316 个网络、1155 个焊盘、4953 条走线、13 个过孔和 557 个连接记录，最新 DRC 中有 550 条未布线。焊盘减少 256 个，恰好对应被删除的第二个 FBGA256 封装。
...
| DRC | 记录新工程真实违规数 | 2621 条活动违规、12 条已记录豁免；详见下方分类，未通过量产验收 |
...
在不移动元器件的前提下，继续补齐了 7 个经网络名和端点确认的断点：DONE 的局部段及 U7-P13 段、PROGRAM B 的局部三段及 U7-T2 段，以及 INIT B 到 U7-R3。每次修改后均完整复跑 DRC，未新增 Clearance、Short-Circuit 或 Hole To Hole 违规。

当前结果为 2621 条活动违规和 12 条已记录豁免：
